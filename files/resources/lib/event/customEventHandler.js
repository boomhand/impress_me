...
function setVar_(key, id, property, src, context){
	...
	localStorage.setItem(key,text);
		...
		if(property == 'visible') {
		    var value = elem.css('display') == 'none' ? 'false' : 'true';
		    localStorage.setItem(key, value);
		}
		else if(property == 'enable') {
		    var value = elem.hasClass('ui-disabled') ? 'false' : 'true';
		    localStorage.setItem(key, value);
		}
		else {
...
function setAttribute_(id, name, val) {
	...
	if(elem.attr("tiggzitype")=="object"){		
		if(elem.attr("name")!=undefined && Tiggr!=undefined){			
			Tiggr(elem.attr("name")).attr(name,val);
			if(Tiggr(elem.attr("name")).refresh!=undefined){
			    Tiggr(elem.attr("name")).refresh();
			}
		}
	}else{
