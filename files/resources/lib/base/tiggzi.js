						...
						if (Tiggr(name) != undefined && Tiggr(name).getOptions != undefined) {								
							if(elem.attr("tiggziclass")!=undefined) {
								if (elem.attr("tiggziclass")=="datepicker") {				
									...
									result = new Tiggr.TiggrMobileDatePickerComponent(elem.attr("id"), options);
								} else if (elem.attr("tiggziclass")=="carousel") {
									result =  new Tiggr.TiggrMobileCarouselComponent(elem.attr("id"));
								} else {
	...
	$t.RenderVisitor = $t.createClass($t.MappingVisitor, {
		...
		beforeArrayVisit: function() {
			...
			if (components.length > 0 && components[0].tagName == "A" && components.eq(0).parents().eq(3).is("[data-role='listview']")) {
				components = components.map(function(ind, elt){return $(elt).parents()[2]});
			}
			components.remove();
		...
		afterArrayVisit: function() {
			...
			if (components[0].tagName == "A" && components.eq(0).parents().eq(3).is("[data-role='listview']")) {
				components = components.map(function(idx, elt){return $(elt).parents()[2];});
			}
		...
		__performHeaderParametersSubstitutions: function(settings) {
			if (!$.isEmptyObject(settings.headers)) {
				...
				for (param in settings.headers) {
					...
					newParam = param.replace(/\{(\w+)\}/g, __performHeaderParametersSubstitution);
					newValue = value.replace(/\{(\w+)\}/g, __performHeaderParametersSubstitution);
...
$t.TiggrMobileDatePickerComponent = $t.createClass($t.TiggrWrapper, {
	...
	$t.refreshScreenFormElements = function(screen) {
		var ctx;
		if (screen) {
		...
		ctx.find("ul[data-role='listview'], ol[data-role='listview']").listview('refresh');
